...
Niri is no longer installed or linked by this repository. The previous Niri
session -- including its tuned config (exact `1366x768@60.016` panel mode,
touchpad workspace scrolling, column-jump and workspace binds, hot-key overlay
titles, hot corner, and focus-ring/border styling) -- is preserved on the
remote branch
[`backup/niri-setup`](https://github.com/watersniffer/Dotfiles-mine/tree/backup/niri-setup).
The earlier Hyprland snapshot is kept on
